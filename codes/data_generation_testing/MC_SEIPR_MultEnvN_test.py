...
sim_index = 0 # from simulation tracking sheet
numSims = 50          
block_size = 50
null_offset = 0
LOW_FREQ_POWER = True
...
def save_csv(i, df_ews, sim_index,regime): 
    """Save EWS and residuals for simulation i"""
    
    # Filter for this simulation with Variable='I'
    df_ews_single = df_ews[(df_ews['tsid'] == i) & (df_ews['Variable'] == 'I')].copy()
    
    if df_ews_single.empty:
        print(f"Warning: No data for tsid={i}")
        return
    
    # Select columns and reset index
    df_ews_single = df_ews_single[['Time', 'residuals', 'ac1', 'variance']].reset_index(drop=True)
    
    # Add global unique tsid
    df_ews_single.insert(0, 'tsid', sim_index + i)
    
    # Master EWS file path (SAME for all batches)
    filepath_ews = f'./SEIPR_MultNoise/data/ews/df_ews_SEIPR_MultNoise_{regime}.csv'
    
    # Check if file exists
    file_exists = os.path.isfile(filepath_ews)
    
    # Write or append
    df_ews_single.to_csv(
        filepath_ews,
    ...
    # Individual resids file
    df_resids = df_ews_single[['Time', 'residuals']].copy()
    filepath_resids = f'./SEIPR_MultNoise/data/resids/resids_SEIPR_MultNoise_{regime}{sim_index+i}.csv'
    df_resids.to_csv(filepath_resids, index=False)
    
    if i % 500 == 0:
        print(f"  Saved {i}/{2500} simulations")
...
#model parameters
Lambda = 100 if not LOW_FREQ_POWER else 150                                     
S0_init = 500                                   
I0_init = 2
E0_init = 2
P0_init = 1
...
df_ews_flat = df_ews_flat[
    ["tsid", "Variable", "Time", "state", "residuals", "ac1", "variance"]
]

if not os.path.exists('./SEIPR_MultNoise/data/ews'):
    os.makedirs('./SEIPR_MultNoise/data/ews')

if not os.path.exists('./SEIPR_MultNoise/data/resids'.format(count)):
    os.makedirs('./SEIPR_MultNoise/data/resids'.format(count))

print('generating labels and tseries csvs')
for i in range(1,numSims+1):
    save_csv(i, df_ews=df_ews_flat, sim_index=sim_index,regime='forced')
...
print('generating labels and tseries csvs')
for i in range(1,numSims+1):
    save_csv(i, df_ews=df_ews_flat, sim_index=sim_index,regime='null')